...
const SHORT_SECTIONS = {"1":["① 生命保険会計の意義および特徴"],"2":["① 静的なソルベンシー検証","① 動的なソルベンシー検証"],"3":["① 契約者配当の四原則","② 利源別配当方式","② アセットシェア方式"],"4":["① 利源枠・純保枠のメリットとデメリット","② EVの意義・考え方・特徴"],"5":["① 区分経理の意義","② 新商品について新区分を設ける理由"],"6":["① VaRのデメリットとストレステストの意義・目的"],"7":["① 責任準備金の長期性による特徴・基礎率の評価性","② ロックイン方式のデメリットを補完する現行制度"],"8":["① リスク管理プロセスの六段階","① 回避・受容・軽減・移転の四カテゴリー"],"9":["（ア）利源分析の意義","（イ）予定利息の内容と役割","（イ）解約・失効契約の消滅時保険料積立金"],"10":["（ア）ソルベンシー評価の意義","（イ）経済価値ベース評価のメリット","（イ）経済価値ベース評価のデメリット"],"11":["（ア）予定事業費枠の意義と役割","（イ）商品別原価計算の目的と概要"],"12":["（ア）金利リスク以外の三つの市場リスク","（イ）流動性リスクと潜在的要因"],"13":["（ア）契約者配当を行う理由"],"14":["（ア）経済価値ベース保険負債評価の概要と法定会計との差","（イ）死亡率前提上昇の影響","（イ）解約率前提上昇の影響"],"15":["（ア）内部管理会計の意義と必要性","（イ）新商品発売当初の費差損益","（イ）新商品発売当初の責任準備金関係損益"],"16":["（ア）ストレステストの意義・目的"]};
const FALLBACK_GROUPS = {"1":["1．現行会計下における、企業活動実態を把握するための管理会計の必要性","2．生命保険会社のグローバル展開が進む中、国際的に統一された会計基準を導入する意義および留意点"],"2":["1．ソルベンシー評価の意義","2．適切な責任準備金評価","3．財務上の備え・リスク対応能力","4．経営への活用"],"3":["A．商品特性の相違を踏まえた配当","B．金利低下傾向における取扱い","C．基礎率改定時の新旧契約間調整","D．発売後年数が短い医療保険の配当率","E．総合判断"],"4":["A．純損失の要因および今後の見通し","B．事業費管理のあり方","C．単年度収支・将来収支・EVの留意点","D．具体的な収益管理手法の提案"],"5":["A．区分ごとの損益把握方法","B．全社区分との関係","C．区分ごとのリスク管理","D．契約者間公平とセルフサポート"],"6":["A．パンデミックシナリオの設定方法","B．ストレステストにおける確認項目","C．パンデミックが確認項目に与える影響","D．テスト結果の活用方法"],"7":["A．ロックフリー方式における計算基礎率の設定","B．資産評価・収益管理","C．ソルベンシー管理","D．ガバナンス・移行管理"],"8":["A．医療保険販売によるリスクプロファイルの変化","B．変化したリスクへの対応","C．リスク管理高度化のための検討事項"],"9":["A．医療保険の特性を踏まえた利源分析","B．追加責任準備金の利源分析上の取扱い","C．分析結果の内部管理・経営施策への活用"],"10":["A．内部管理目的でのソルベンシー評価","B．健全性確保の方策とリスク管理","C．収益性向上への活用"],"11":["A．事業費・事業費効率を把握する一般論","B．インフレ率上昇を踏まえた留意点","C．IT投資等の経営政策上の留意点"],"12":["A．考慮すべき保険契約者のオプション","B．資産ポートフォリオ・運用方針","C．流動性管理と継続的リスク管理"],"13":["A．商品特性の相違を踏まえた配当","B．新旧予定利率契約の利差配当水準","C．キャピタルゲイン還元","D．総合判断"],"14":["A．経済価値評価の意義と留意点","B．非経済前提の設定方法と留意点","C．前提・モデル・結果の妥当性向上と理解促進"],"15":["A．新商品を区分経理する意義","B．損益把握上の留意点","C．チャネル・商品間の相乗効果の評価"],"16":["A．商品別・全社的な主要リスク","B．ストレスシナリオの設定","C．テスト結果の活用方法"]};
  ...
  return <div className={styles.answerView}>
    <section className={styles.section}>
      ...
      <div className={styles.text}>
        {prepared.groups.map((group) => <section className={styles.majorGroup} key={group.title}>
